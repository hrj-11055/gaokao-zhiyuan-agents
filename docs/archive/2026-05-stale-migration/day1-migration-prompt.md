...
> 用途：复制此提示词给 Claude Code，指导完成新服务器迁移 + Dify 验证 + 30 题测试框架搭建
...
| # | 意图类别 | 题数 | 示例 |
|---|---------|------|------|
| 1 | 志愿填报策略 | 6 | 「冲稳保怎么分配比例？」 |
| 2 | 院校推荐/分数线 | 6 | 「广东 580 分物理类能考虑哪些大学？」 |
| 3 | 专业分析 | 6 | 「人工智能专业学什么？就业怎么样？」 |
| 4 | 测评/报告联动 | 4 | 「MBTI 是 INTJ，霍兰德偏 I/C，适合什么专业？」 |
| 5 | 边界与异常 | 4 | 「我考了 0 分怎么办？」 |
| 6 | 张雪峰风格一致性 | 4 | 「女生学计算机好吗？」 |

请帮我生成这 30 题的完整列表，保存到 `data/test-runs/30q-test-set.md`。
...
1. 用 gaokao-proxy `/api/chat` 逐条发送 30 题
2. 每条回复由 LLM（Claude/Gemini）自动初评 4 维度分数
3. 自动初评后，我人工抽查 10 题校准
4. 输出：
   - `data/test-runs/30q-results.md` — 逐题评分明细
   - `data/test-runs/30q-raw-results.json` — 原始回答

### 7.4 自动化脚本

请帮我编写 `data/run_test_30q.py`，功能：
- 读取 `data/test-runs/30q-test-set.md` 中的 30 题
- 逐题调用 Dify API（通过 gaokao-proxy 的 /api/chat 接口，阻塞模式）
- 每题间隔 3 秒（避免限流）
- 记录每题的 question、answer、耗时
- 保存到 `data/test-runs/30q-raw-results.json`

然后可选编写 `data/eval_30q.py`，功能：
- 读取 raw results
...
- 30 题测试集的自动跑批约 10-20 分钟（3s 间隔 × 30 题 + API 响应时间）
