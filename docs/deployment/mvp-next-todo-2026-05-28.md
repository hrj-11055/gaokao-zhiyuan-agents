...
- 2026-05-28 SSH 检查发现 47 服务器仍是 `MEMBERSHIP_PRICE_CENTS=100`，需要上线前改为 `1990`。
- 2026-05-28 SSH 检查发现 47 服务器仍是 `DEEPSEEK_MODEL=deepseek-chat`，需要改为 `deepseek-v4-pro`。
...
- [ ] 将 47 服务器 `/opt/gaokao-proxy/.env` 改为 `MEMBERSHIP_PRICE_CENTS=1990`。
- [ ] 将 47 服务器 `/opt/gaokao-proxy/.env` 改为 `DEEPSEEK_MODEL=deepseek-v4-pro`。
...
- [x] 用户可一键复制微信号。
- [ ] 客服话术补齐：支付成功未解锁、重复扣款、邀请码无效、PDF 次数耗尽、报告生成失败。
- [ ] 明确客服需要用户提供：用户 ID、支付截图、问题截图、发生时间。

## P2：登录真机验证

- [ ] 正式体验版确认 `VITE_WECHAT_LOGIN_MOCK=false`。
- [ ] 47 服务器确认 `WECHAT_LOGIN_MOCK=0`。
