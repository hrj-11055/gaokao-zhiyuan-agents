...
项目先回到本地开发，不急于上线服务器。小程序主体备案、微信审核类目和 AI 问答资质是上线前置条件，在这些问题没有解决前，不把 `aicoming.com.cn` 作为正式小程序接口域名。
...
如果后期使用中国内地服务器，并用域名访问服务器接口，例如：

```text
https://www.aicoming.com.cn/api/health
```

则域名需要在服务器接入商处完成网站/App 相关 ICP 备案或接入备案。否则云厂商可能直接拦截 Host 请求，表现为：
