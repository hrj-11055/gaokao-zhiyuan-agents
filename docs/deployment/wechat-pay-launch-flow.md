...
- 服务器侧已有 `npm run commerce-ops` 手工排查与补偿 CLI，可按用户 ID、openid、本地订单号、微信商户单号、微信支付订单号查询订单/会员状态，并可补开会员或发放一次性补偿邀请码。
...
- 客服兜底：上线前值班人员必须能在 47 服务器执行 `npm run commerce-ops -- lookup ...`，并按 `docs/deployment/customer-support-playbook.md` 完成补开会员或补偿码 SOP。
