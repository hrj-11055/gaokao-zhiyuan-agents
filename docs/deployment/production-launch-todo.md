...
- 代码与文档当前正式价格统一为 19.9 元：后端 `MEMBERSHIP_PRICE_CENTS=1990`，小程序展示 `¥19.9`。2026-05-28 已把 47 服务器从 1 元测试配置恢复为 `1990`，仍需真机复测 19.9 元支付。
- 支付接口联调仍有若干异常和边界场景待测试，见“支付收尾待办”。
- 已新增 `npm run commerce-ops` 手工排查与补偿 CLI，支持按用户 ID、openid、本地订单号、微信商户单号、微信支付订单号查询订单/会员状态，并支持补开会员和发放一次性补偿邀请码；SOP 见 `docs/deployment/customer-support-playbook.md`。
...
- [ ] 正式上线前已恢复 `MEMBERSHIP_PRICE_CENTS=1990`；仍需完成 19.9 元真机复测。
...
- [x] 手工订单/会员查询：`npm run commerce-ops -- lookup` 支持按用户 ID、openid、订单号、商户单号和微信支付订单号查询。
- [x] 手工补偿 SOP：客服文档已覆盖补开会员和发放一次性补偿邀请码，并要求写入 `support_operations` 留痕。
...
- [ ] 深度报告下载次数限制链路通过。
- [x] 小程序备案已完成。
- [x] 客服可按 SOP 查询订单/会员，并执行补开会员或补偿码发放。
...
- 线上日志和 `commerce-ops` 能通过 orderId / outTradeNo / transactionId 串起一次支付全过程（待 47 部署后复测 PM2 日志与 SQLite 查询）。

### 3. 正式价格恢复与全链路验收

- [x] 将 47 服务器 `MEMBERSHIP_PRICE_CENTS` 恢复为 `1990`。
