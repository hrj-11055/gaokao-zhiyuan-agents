...
你必须直接输出完整的 9 个模块报告内容（Markdown 格式），不要只输出摘要。
不要说"报告已保存"或"报告已生成"，直接输出报告全文。
从"## 模块一：学术资本"开始，到"## 模块九：结构化数据导出"结束，完整输出每一个模块。
不要输出"内部评分标尺"部分。
报告正文不少于 3000 个中文字符；每个模块都要有表格、关键数据、简短分析，禁止压缩成提纲。
...
1. 必须保留并完整输出 9 个模块，从“## 模块一：学术资本”到“## 模块九：结构化数据导出 (JSON)”。
...
def save_report(text: str, univ: dict) -> Path:
    ...
    if json_match:
        json_path = OUTPUT_DIR / f"{univ['name']}.json"
        try:
            json_data = json.loads(json_match.group(1))
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(json_data, f, ensure_ascii=False, indent=2)
        except json.JSONDecodeError:
            print(f"  ⚠ 警告: {univ['name']} 的 JSON 数据格式错误，解析失败")
